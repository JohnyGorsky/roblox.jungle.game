...
| **Animation — WATER creatures** | **REVISED 2026-08-15 (user): no skeleton at all.** A river creature is half submerged and the waterline hides exactly what a rig would animate — the legs and the belly. So a water creature is a static mesh **sunk to the waterline**, with procedural **bob + roll + yaw wag**, a **wake** that only emits while it swims, and a **splash burst** on strike and death. Costs no credits, no rig, no animation uploads, and it sidesteps the real risk that Meshy's auto-rigger is humanoid-oriented and a 20-stud horizontal quadruped is its worst case. Implemented in `EnemyRig` + `EnemyAssets.WaterMotion`/`CreatureVfx`. **Land creatures still want real animation.** |
| **Animation — LAND creatures** | **`AnimationController` + `Animator`, keeping `PivotTo` movement.** Animations are cosmetic only, so the server-authoritative AI tuned across Jobs 006–009, 053 and 058 is not touched. Requires skinned/bone rigs out of Meshy (its auto-rig + animation library). |
